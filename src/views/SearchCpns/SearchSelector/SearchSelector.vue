<template>
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="item in attrsList" :key="item.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(item2, index) in item.attrValueList" :key="index" @click="choseNorms(item,item2)">
            <a>{{ item2 }}</a>
          </li>
        </ul>
      </div>
  ...
  export default defineComponent({
    ...
    setup(props, {emit}) {
      ...
      const moveToTrademark = (item) => {
        emit("moveToTrademark", item);
      };
      const choseNorms = (item, item2) => {
        const props = `${item.attrId}:${item2}:${item.attrName}`
        emit('choseNorms', props)
      }
      return {
        trademarkList,
        attrsList,
